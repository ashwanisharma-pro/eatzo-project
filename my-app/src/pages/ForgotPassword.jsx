...
const ForgotPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Error sending recovery request');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="auth-page-v3">
      ...
      <div className="auth-split-wrapper">
        ...
        <div className="auth-brand-side">
          <img src={authBg} alt="Account Security" className="brand-bg-image" />
          <div className="brand-overlay"></div>
          
          <div className="brand-content">
            <h2>Account <span>Recovery.</span></h2>
            <p>Don't worry, it happens to the best of us. Let's get you back to your culinary journey at Eatzo.</p>
          </div>
          ...
          <div className="form-container">
            ...
            {!submitted ? (
              ...
              <div style={{ textAlign: 'center', padding: '40px 0' }}>
                ...
                <h4 style={{ color: '#fff', fontSize: '1.5rem', fontWeight: '800', marginBottom: '16px' }}>Check Your Inbox</h4>
                <p style={{ color: '#94a3b8', lineHeight: '1.6' }}>
                  We have sent a secure password reset link to: <br/> 
                  <strong style={{ color: '#fff' }}>{email}</strong>
                </p>
                <button onClick={() => setSubmitted(false)} className="btn-auth-full" style={{ background: 'rgba(255, 255, 255, 0.05)', color: '#fff', border: '1px solid rgba(255, 255, 255, 0.1)', marginTop: '30px' }}>
                  Try another email
                </button>
              </div>
            )}
            ...
            <div style={{ marginTop: '40px', display: 'flex', alignItems: 'center', gap: '15px' }}>
              ...
              <span style={{ fontSize: '0.85rem', color: '#64748b' }}>Response time typically under 2 minutes.</span>
            </div>
          </div>
        </div>
      </div>
